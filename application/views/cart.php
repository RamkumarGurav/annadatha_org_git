...
<div>
				...
				<div class="cart1 mb-4 col-lg-8">
					<div class="row align-items-center">
						<div class="col-lg-6 col-6">
							...
							<div><?php echo $this->session->userdata('application_sess_cart_page_address'); ?></div>
						</div>

						<div class="col-lg-6 col-6">

							<a href="" data-bs-toggle="modal" data-bs-target="#func_pin_location_modal_icon"
								class="delivery-pincode">Enter Delivery Pincode</a>
	...
	<script>
		function myFunction() {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url(__payment__) ?>",
				dataType: "json",
								...
								if (response.status == "userCancelled") {
									...
									$.ajax({
										...
										success: function (result) { //alert(result.status);
											...
											if (result.status == 1) {
												window.location = "<?php echo base_url() ?>order_status";
											}
											else {
												window.location = "<?php echo base_url() ?>order_status";
											}
	...
	<div class="modal fade" id="func_pin_location_modal_icon" tabindex="-1" aria-labelledby="exampleModalLabel"
		aria-hidden="true">
				...
				<? if (_shipping_show_service_availablity == 1) { ?>
					<div class="use-pincode _shipping_show_service_availablity_cl">
						...
						<div class="pincode-input-content">
							<input placeholder="Enter pincode" type="text" maxlength="6" minlength="6" name="pincode_d" id="pincode_d"
								autocomplete="off" class="pincode-input" value="">
	...
	<script>
		...
		window.addEventListener('load', function () {
			$("#func_pin_location_modal_icon").on('show.bs.modal', function () {
				...
				$.ajax({
					type: "POST",
					url: '<?php echo MAINSITE ?>Products/getCartpageAddress/',
					data: {},
					success: function (result) {
