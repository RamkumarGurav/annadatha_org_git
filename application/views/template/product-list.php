    ...
    foreach ($products_list as $col) {
        ...
        <?  if ($callFor == 'slider') { ?>
            ...
            <?  } else { ?>
                ...
                <div class="product-miniature col-lg-<?php  echo $_prow_count ?> col-<?php  echo $_prow_countm ?>">
                <?  } ?>
                ...
                <div class="js-product product product-card">
                    ...
                    <div class="thumbnail-container reviews-loading">
                        <div class="ttproduct-image ttpc-parent">
                            <a href="<?php  echo $product_link ?>" title="<?  echo $product_name; ?> ">
                                <img class="ttproduct-img1 img-fluid lazy_product "
                                    data-src="<?php  echo _uploaded_files_ ?>product/medium/<?  echo $product_image_name; ?>"
                                    data-page_count="<?  echo $page_count; ?>" alt="<?  echo $product_name; ?>"
                                    title="<?  echo $product_name; ?>"
                                    data-full-size-image-url="<?php  echo _uploaded_files_ ?>product/medium/<?  echo $product_image_name; ?>"
                                    data-callfor='<?php  echo $callFor ?>'>
                            </a>
                            ...
                            <div class="ttproducthover ">
                                ...
                                <?  if (!empty($discount) && $discount > 0) { ?>
                                    <div class="ribbon ribbon-top-left"><span><?php  echo $discount ?> off</span></div> <?  } ?>

                                <!-- <?  if (!empty($discount) && $discount > 0) { ?><span class="dis_section"> <span><?php  echo $discount ?></span><div class="clearfix"></div>OFF</span><?php  } ?> -->
                                <div class="wishlist">
                                    <span class="addToWishlist">
                                        <!--<i class="material-icons">favorite_border</i>-->
                                        <a class="cart_wishlist_<?  echo $product_in_store_id; ?> dark-bg cart_wishlist_n_<?  echo $product_in_store_id; ?> addToWishlistListBTN<?  echo ($offset != 1) ? $offset : '' ?>"
                                            ...
                                            style="cursor: pointer; <?  if ($prod_in_wishList <= 0) {
                                                echo " display:inline-block";
                                            } else {
                                                echo "display:none";
                                            } ?> "><img
                                                src="<?php  echo IMAGE ?>p-like.png" alt="" class="p-ic"></a>

                                        <a class="cart_wishlist_<?  echo $product_in_store_id; ?> light-bg cart_wishlist_y_<?  echo $product_in_store_id; ?> addToWishlistListBTN<?  echo ($offset != 1) ? $offset : '' ?>"
                                            ...
                                            style="cursor: pointer; <?  if ($prod_in_wishList >= 1) {
                                                echo " display:inline-block";
                                            } else {
                                                echo "display:none";
                                            } ?> "><img
                                                src="<?php  echo IMAGE ?>pd-like.png" alt="" class="p-ic"></a>
                                    </span>

                                    <a style="cursor: pointer;"
                                        onclick="product_quick_view_func(<?php  echo $product_id ?> , <?php  echo $product_combination_id ?>)"
                                        title="Quick View - <?  echo $product_name; ?> - <?  echo str_replace('&nbsp;', ' ', $combi); ?>"
                                        class="addToview">
                                        <img src="<?php  echo IMAGE ?>p-zoom.png" alt="" class="p-ic">
                                    </a>
                                </div>
                            </div>
                        </div>
                        ...
                        <a href="<?php  echo $product_link ?>" title="<?  echo $product_name; ?> ">
                            <div class="product-description">
                                ...
                                <div class="product-price-and-shipping">
                                    <span class="price"><?php  echo $currency->symbol ?>
                                        <?  echo number_format($final_price) ?></span>
                                    <?  if (!empty($discount) && $discount > 0) { ?><span
                                            class="sr-only">Price</span>&nbsp;<strike><?php  echo $currency->symbol ?>
                                            <?  echo number_format($price) ?></strike>
                                        <span class="pdisc dspblck">Save <?php  echo $currency->symbol ?>
                                            <?php  echo number_format($discounted_price) ?> Off</span><?php  } ?>
                        ...
                        <?php  if ($in_store_quantity > 0) { ?>
                            ...
                            <div class=" productAddShow_<?  echo $product_in_store_id; ?>" <?  if (!empty($prod_in_cart) || $prod_in_cart > 0) { ?> style="display:none;" <?  } ?>>
                                <button type="button"
                                    ...
                                    class=" adtocart ad_crt2_btn addToCartListBTN<?  echo $offset ?>"
                                    data-val="<?  echo $product_in_store_id; ?>,<?  echo $product_id; ?>,<?  echo $product_combination_id; ?>,2"><i
                                        class="fa fa-shopping-cart"></i> Add to Cart</button>
                            </div>

                            <div class=" go_to_cart qty_increDecre_<?  echo $product_in_store_id; ?>"
                                style="display:<?  if (!empty($prod_in_cart) || $prod_in_cart > 0) {
                                    echo " block";
                                } else {
                                    echo "none";
                                } ?>">
                                <a <?php  /*?>href="<?php  echo __cart__?>"<?php  */ ?> onclick="cartopenNav()" target="_blank"
                                    class="adtocart selected_c" title="Go to cart">Go to cart</a>
                            </div>
...
<?  } else { ?>
    ...
    <div class="row " style="clear: both;padding: 20px;font-weight: 600;font-size: 18px;text-shadow: 1px 4px 6px #b1b1b1;">
        <?php  /*?>No product found. Please refine search criteria<?php  */ ?>
        <div class="no_prd_found"><span>Sorry!</span> No Product Found</div>
        <img src="<?php  echo 'assets/front/images/no-product.jpg' ?>" class="responsiveImg">
    </div>
