    ...
    foreach ($products_list as $col) {
      ...
      foreach ($col['product_combination'] as $row) {
        if ($row['discount_var'] == 'Rs') {
          $discount = $currency->symbol . ' ' . $CI->getCurrencyPrice(array('obj' => $this->data, 'amount' => $row['discount']));
      ...
      <div class="proditem" infinite-scroll="">
        <div class="leftside-icons">
          <a class="product-item-photo aClick_2 aClick_1" href="<?php  echo $product_link ?>" title="">
            <img class="pro-img" width="100px" height="100px" src="<?php  echo $SimagePath . $product_image_name ?>"
              alt="<?php  echo $product_name ?>" title="<?php  echo $product_name ?>"></a>
        </div>

        <div class="rightside-details">
          <div class="row m-0">
            <div class="product-item-name col-lg-8 col-md-6 pl-0">
              <a href="<?php  echo $product_link ?>" title=""
                class="aClick_2 aClick_1"><?php  echo $product_name ?><br><small><?php  echo $combi ?></small></a>
            </div>

            <div class="item-prices col-lg-4 col-md-6 p-0 text-right">
              <span class="itemprice"><?php  echo $currency->symbol . ' ' . $final_price ?></span>
              <?  if (!empty($discount) && $discount > 0) { ?>
                <span class="price_old"><?  echo $discount . " OFF"; ?></span>
                <span class="mrpprice">M.R.P: <span><?php  echo $currency->symbol . ' ' . $price ?></span></span>
              <?  } ?>
            </div>
          </div>

          <div class="row m-0">
            <div class="mfr-name col-lg-7 col-md-6 pl-0"><!---->
              <p _ngcontent-ynj-c9="" class="m-0 mt-1">By: <a href="<?php  echo $product_link ?>"><?php  echo $brand_name ?></a>
              </p>
            </div>
            <div class="addtocartbox col-lg-5 col-md-6 p-0 text-right"><!----><!----><!----><!----><!---->
              ...
              <button type="button" class="cart_buton3 bg btn btn-default WishlistListBTN move-to-cart-btn"
                data-val='<?php  echo $product_in_store_id ?>,<?php  echo $product_id ?>,<?php  echo $product_combination_id ?>,4'>Move
                to Cart <i class="fa fa-shopping-cart"></i></button>

              <a class="delete WishlistListBTN btn btn-danger aClick_2 aClick_1"
                data-val="<?php  echo $product_in_store_id ?>,<?php  echo $product_id ?>,<?php  echo $product_combination_id ?>,2"><i
                  class="fa fa-trash"></i></a>


              <div class="remove-drug mt-2 d-none">
                <a class="action action-delete removeitem aClick_2 aClick_1" href="javascript:void(0);"
                  title="Remove item">Remove</a>
              </div>
            </div>
          </div>
        </div>
...
<img src="<?php  echo __scriptFilePath__?>images/icons/heart.png" class="empty_notif_2">

 <span class="empty_notif_1">your wishlist is empty</span>
 <a href="<?php  echo base_url()?>all-products" class="empty_notif_3"><button class="proceed_ckt"><i class="fa fa-shopping-cart"></i> <span> Continue Shopping</span></button></a>
</article><?php  */ ?>

  <article class="cart_tabs_1 clearfix col-md-12 cart-indi wishlistPageEmpty">
    <img src="<?php  echo __scriptFilePath__ ?>images/empty-wishlist.png" class="empty_notif_2">

    <!--<span class="empty_notif_1">your wishlist is empty</span>-->
    <a href="<?php  echo base_url() ?>all-products" class="empty_notif_3"><button class="proceed_ckt"><i
          class="fa fa-shopping-cart"></i> <span> Continue Shopping</span></button></a>
  </article>
