<script>
	...
	document.getElementById('current_viewers_msg').innerHTML = "<img src='<?php echo __scriptFilePath__ ?>images/icons/eye.svg'> Currently " + view_product + " people are viewing this product";
</script>
<? if ($check_screen == 'isdesktop') { ?>
	<script src="<?php echo __scriptFilePath__ ?>js/Drift.min.js"></script>
	<script>
		<? $picount = 0;
		foreach ($products_image as $pi) {
			$picount++; ?>
			var test = new Drift(document.querySelector('.drift-demo-trigger<?php echo $picount ?>'), {
...
<script>
	...
	function SubmitRatingForm(product_id) {
		...
		var mainsite = "<?php echo base_url() ?>";
	...
	function validate_bulk_enquiry_form() {
		...
		if (count > 0) {
			...
			$.ajax({
				...
				url: '<?php echo base_url() ?>user/bulk_enquiry_product',
