...
$(document).ready(function() {
   ...
   type: "POST",
	...
	url:'<?php  echo MAINSITE_Admin ?>catalog/Category-Module/GetCompleteCategoryList',
   //dataType : "json", 
   data : {"super_category_id" : '<?php  echo $category_id; ?>' , "withPosition" : 1  , 'sortByPosition':1 , "<?php  echo $csrf['name']?>":"<?php  echo $csrf['hash']?>"},
                        ...
                        <div class="col-md-12 card-body ">
                            <div class="box box-primary">
                                <div class="box-header with-border">
                                	<h3 class="box-title">Category Details <?php  if($category_id!='parents'){ ?>
									<?php   if($user_access->update_module==1)	{ ?>
									<a href="<?php  echo MAINSITE_Admin.$user_access->class_name?>/edit/<?php  echo $category_id?>" class="btn btn-primary" style="float :right;margin-top: -5px">Update Category Details
									</a>
									<?php  } ?>
									<?php  } ?></h3>
                                </div>
								...
								<?php   if($user_access->view_module==1)	{ ?>
                                     <table id="" class="table  table-hover myviewtable responsiveTableNewDesign">  
                                        ...
                                        <?php  if(!empty($category_icon)){ ?><a href="<?php  echo _uploaded_files_.'category/'.$category_icon; ?>" target="_blank"><img width="150" src="<?php  echo _uploaded_files_.'category/'.$category_icon; ?>" /></a><?php  } ?></td>
                                        <td >
                                        <strong class="full">Cover Image</strong>
                                        <?php  if(!empty($cover_image)){ ?><a href="<?php  echo _uploaded_files_.'category/'.$cover_image; ?>" target="_blank"><img width="150" src="<?php  echo _uploaded_files_.'category/'.$cover_image; ?>" /></a><?php  } ?></td>
                                        
                                                        </tr>
                                                        <tr>
                                                            <td colspan="1">
                                        <strong class="full">Header 1 Image</strong>
                                        <?php  if(!empty($cover_image)){ ?><a href="<?php  echo _uploaded_files_.'category/'.$header_1_img; ?>" target="_blank"><img width="150" src="<?php  echo _uploaded_files_.'category/'.$header_1_img; ?>"" /></a><?php  } ?></td>
                                        <td colspan="3">
...
<div class="tableDemo">
                                ...
                                <img  src="<?php  echo _uploaded_files_."load.gif"; ?>" />
                                </div></td>
                                    </tr>
	 ...
	 table_2 = $("#table-2");
	    ...
	    onDragClass: "myDragClass",
			onDrop: function(table, row) {
				...
				$('#stateList').html('<tr><td colspan="10"> <div class="clearfix text-center" ><img  src="<?php  echo _uploaded_files_."load.gif"; ?>" /></div></td></tr>');
				$.ajax({
			   type: "POST",
			   url: '<?php  echo MAINSITE_Admin.'catalog/Category-Module/GetCompleteCategoryListNewPos'?>',
			   //dataType : "json", 
			   data : {"super_category_id" : '<?php  echo $category_id; ?>' , 'podId':podId , "<?php  echo $csrf['name']?>":"<?php  echo $csrf['hash']?>"},
			   success : function(result){
				  // alert(result);
					$('#stateList').html(result);
