...
$filename = "Coupon-List-" . date('d-m-Y') . ".xls";
    ...
    <table width="100%" border="1" align="center" cellpadding="0" cellspacing="0">
        <thead>
            ...
            <tr>
                ...
                <th style="background-color:#999" width="*">Coupon Name</th>
                <th style="background-color:#999" width="*">Discount In</th>
                <th style="background-color:#999" width="*">Discount Value</th>
        ...
        <tbody>
            ...
            if (!empty($coupon_data)) { //print_r($ptype_list);
                foreach ($coupon_data as $row) {
                    //if($row->in_hand > 0 || $row->total_purchase > 0 || $row->total_sold > 0 || $row->total_returned > 0 || $row->to_receive > 0 )
                    {
                        ...
                        <tr>
                            ...
                            <td width="*"><?  if ($row->discount_in == 1) { ?> Rs.<?  } else { ?>%<?  } ?></td>

                            <td width="*"><?  echo $row->discount_value; ?></td>
