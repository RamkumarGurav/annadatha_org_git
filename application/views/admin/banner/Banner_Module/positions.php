    ...
    <? php if ($user_access->view_module == 1) { ?>
        $(document).ready(function () {
            $.ajax({
                ...
                url: '<?  echo MAINSITE_Admin ?>banner/Banner-Module/GetCompleteBannerList',
                //dataType : "json",
                data: { "banner_id": '<?  echo $banner_id; ?>', "withPosition": 1, 'sortByPosition': 1, "<? php echo $csrf['name'] ?>": "<? php echo $csrf['hash'] ?>" },
                        ...
                        function dragEvent() {
                            ...
                            $("#table-2").tableDnD({
                                ...
                                onDrop: function (table, row) {
                                    ...
                                    $('#bannerList').html('<tr><td colspan="10"> <div class="clearfix text-center" ><img  src="<?  echo MAINSITE . "assets/admin/images/load.gif"; ?>" /></div></td></tr>');
                                    $.ajax({
                                        type: "POST",
                                        url: '<? php echo MAINSITE_Admin . 'banner/Banner-Module/GetCompleteBannerListNewPos' ?>',
                                        //dataType : "json",
                                        data: { "banner_id": '<?  echo $banner_id; ?>', 'podId': podId, "<? php echo $csrf['name'] ?>": "<? php echo $csrf['hash'] ?>" },
                                        success: function (result) {
                                            // alert(result);
                                            $('#bannerList').html(result);
