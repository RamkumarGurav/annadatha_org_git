...
<?php if (!empty($response)) {
  ...
  if (!empty($response->status)) {
    if ($response->status == 404) {
      ?>
      <div class="alert alert-warning alert-dismissible"><i class="icon fas fa-ban"></i><?php echo $response->message ?>
      </div>
      <?php
      exit;
    }
      ...
      <div class="card-header panel-heading">
        <h4 class="card-title panel-title">
          <a data-toggle="collapse" data-parent="#accordion" href="#collapse1"
            style="border-top: none !important">Dispatch Order Through Shiprocket</a>
        </h4>
      </div>
            ...
            <tbody>
              ...
              foreach ($response->data->available_courier_companies as $r) {
                ...
                <tr>
                  ...
                  <td><?php echo $r->etd ?> ( <?php echo $r->estimated_delivery_days ?> Days )</td>

                  <td><?php echo $r->cod_charges ?></td>

                  <td><?php echo $r->freight_charge ?></td>
                  <td><?php echo $r->rate ?></td>
                  <td><button type="button"
                      onclick="assignShiprocketOrderAWB(<?php echo $r->courier_company_id ?> , '<?php echo $r->rate ?>')"
                      class="btn btn-primary assignShiprocketOrderAWBBTN">Assign Docket</button></td>
                </tr>
              <?php } ?>
