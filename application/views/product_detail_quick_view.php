  ...
  <div class="row prodct_wrap">
    <div class="col-md-6 prdt-sticky-top">
      <div class="Product-popImg">
        <?  foreach ($products_image as $pi) { ?>
          <img src='<?php  echo $LimagePath . $pi['product_image_name'] ?>' class="image"
            alt="<?php  echo $product_name ?>" title="<?php  echo $product_name ?>">
        <?  } ?>
      </div>
    </div>
    <div class="prodct_det2 product_cnt col-md-6">
      <h2 class="prodt_head1"><?php  echo $product_display_name ?></h2>
      <div class="rate_setion">
        <div class="rate">
          ...
          </div> <span class="rating_valu">(<?php  echo $totalreview ?>)</span>
        </div>
      </div>
      ...
      <?  if (!empty($product_use_info)) { ?>
        ...
        <ul class="prdoct_fetr">
          <?  foreach ($product_use_info as $pui) { ?>
            <li><img
                src="<?php  echo base_url() ?>assets/front/images/product-use-info-icons/<?php  echo $pui->image ?>"><?php  echo $pui->content ?>
            </li>
          <?  } ?>
        </ul>
      <?  } ?>
      <div class="ProdtDescHeight">
        ...
        <p class="main_parag2"> <?php  echo $short_description ?>
        </p>
          ...
          if ($default_product_combination_id != $r_pc['product_combination_id']) {
            ...
            if ($product_combination_id == $_POST['product_combination_id']) {
              ...
              <div class="product_sec1">
                <h4>offer_tag_quick<?php  echo $r_product_name ?> - <?php  echo $r_combi ?></h4>
                <h4 class="prodct_prce">
                  <?  if (!empty($discount)) { ?><span class="old_price"><?php  echo $currency->symbol ?>
                      <?  echo $price ?></span><?  } ?>
                  <span class="new_price"><?php  echo $currency->symbol ?>         <?  echo $final_price ?></span>
                  <span class="offer_tag_quick"><?  if (!empty($discount)) { ?>           <?php  echo $discount ?> OFF<?  } ?></span>
                  ...
                  <?  if ($in_store_quantity > 0) { ?>
                    ...
                    <div class="disply_inline mar-l-r-0 productAddShow_10 prdt_cost_right" style="">
                      ...
                      <div class="disply_inline input_group plus-minus-input">
                        <div class="pad-mar-l-r-0 productAddShow_<?  echo $product_in_store_id; ?>" <?  if (!empty($prod_in_cart) || $prod_in_cart > 0) { ?> style="display:none" <?  } ?>><button
                            class=" crt_button2 ad_cart_btn addToCartListBTN<?  echo $offset ?>"
                            data-val="<?  echo $product_in_store_id; ?>,<?  echo $product_id; ?>,<?  echo $product_combination_id; ?>,2"><i
                      ...
                      <div class="disply_inline pad-mar-l-r-0 productInCartShow_<?  echo $product_in_store_id; ?>" <?  if (!empty($prod_in_cart) || $prod_in_cart > 0) { ?> style="display:inline-block" <?  } else { ?>style="display:none" <?  } ?>>
                        <?php  /*?><a href="<?php  echo base_url().__cart__?>"><button class="ad_cart_btn "> Go To Cart</button></a><?php  */ ?>
                      ...
                      <div class="disply_inline input_group plus-minus-input" style="margin-left:10px;">
                        <button class="crt_button1 addToCartListBTN<?  echo $offset ?>"
                          data-val="<?  echo $product_in_store_id; ?>,<?  echo $product_id; ?>,<?  echo $product_combination_id; ?>,8">Buy
                          Now</button>
                      </div>
                    </div>
                  <?  } else { ?>
                    <div>
                      ...
                      <input class="profil_form email-notification notfy_mail" type="email" name="notify_email" id="notify_email"
                        value="<?  if (!empty($customers->email)) {
                          echo $customers->email;
                        } ?>" />
                      <span class="error_span"></span>

                      <button class="notify_btn1 notify_buton addToNotifyMeListBTN<?  echo $offset ?>"
                    ...
                    <a class="crt_button3 wishlist-i cart_wishlist_<?  echo $product_in_store_id; ?> cart_wishlist_n_<?  echo $product_in_store_id; ?> addToWishlistListBTN<?  echo $offset ?>"
                      data-val="<?  echo $product_in_store_id; ?>,<?  echo $product_id; ?>,<?  echo $product_combination_id; ?>,1"
                      title="Add to Wishlist - <?  echo $product_name; ?> - <?  echo str_replace('&nbsp;', ' ', $combi); ?>"
                      style=" <?  if ($prod_in_wishList <= 0) {
                    ...
                    <a class="crt_button3 wishlist-i cart_wishlist_<?  echo $product_in_store_id; ?> cart_wishlist_y_<?  echo $product_in_store_id; ?> addToWishlistListBTN<?  echo $offset ?>"
      ...
      <div class="col-md-12 text-right prdt-sticky-btm">
        <a href="<?php  echo $og_url ?>" class="themeBtn"
          title="View More - <?  echo $product_name; ?> - <?  echo str_replace('&nbsp;', ' ', $combi); ?>">View More</a>
      </div>
    </div>
  </div>
</div>
  ...
  waShBtn = function () {
    if (this.isIos === true) {
      ...
      for (var i = 0; i < b.length; i++) {
        var t = b[i].getAttribute("data-text", '<?php  echo $og_title ?>');
        var u = b[i].getAttribute("data-href", '<?php  echo $og_url ?>');
