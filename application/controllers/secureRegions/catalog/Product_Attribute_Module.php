...
class Product_Attribute_Module extends Main
{
	...
	function listing()
	{
		...
		// echo "<pre> <br>";
		// print_r($this->data['product_attribute_data']);
		// exit;
	...
	function doEdit()
	{
		...
		if ($_POST['redirect_type'] == 'save-add-new') {
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/" . $user_access->function_name . "/edit" . $msg);
		}
		...
		if (count($attribute_value_list) != 0) {
			foreach ($attribute_value_list as $row) {
				...
				if (!empty($row->color_name)) {
					$color_val = '&nbsp;&nbsp;&nbsp;<span style="background-color:' . $row->color_name . '">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>';
				}
				...
				if ($row->status) {
					$show .= "<td align='center'><i class='fa fa-check true-icon'></i><span style='display:none'>Publish</span></td>";
				} else {
