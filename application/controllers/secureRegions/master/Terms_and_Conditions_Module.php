...
class Terms_and_Conditions_Module extends Main
{
	...
	function __construct()
	{
		...
		$this->load->model('administrator/master/Terms_and_Conditions_Model');
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Terms_and_Conditions_Module/list', $this->data);
		parent::get_footer();
	}

	function terms_and_conditions_list()
	{
		...
		$data_count = $this->Terms_and_Conditions_Model->get_terms_and_conditions_master($search);
		...
		$this->data['terms_and_conditions_master_data'] = $this->Terms_and_Conditions_Model->get_terms_and_conditions_master($search);

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Terms_and_Conditions_Module/terms_and_conditions_list', $this->data);
		parent::get_footer();
	}

	function terms_and_conditions_list_export()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 13;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));
		...
		$this->load->view('admin/master/Terms_and_Conditions_Module/terms_and_conditions_list_export', $this->data);
	}

	function terms_and_conditions_view($terms_and_conditions_id = "")
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 13;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));
		//print_r($this->data['user_access']);
		if (empty($terms_and_conditions_id)) {
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['terms_and_conditions_master_data'] = $this->Terms_and_Conditions_Model->get_terms_and_conditions_master(array("terms_and_conditions_id" => $terms_and_conditions_id));
		if (empty($terms_and_conditions_id)) {
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Something Went Wrong. Please Try Again. anubhav</div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/" . $user_access->function_name);
		...
		$this->data['terms_and_conditions_master_data'] = $this->data['terms_and_conditions_master_data'][0];

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Terms_and_Conditions_Module/terms_and_conditions_view', $this->data);
		parent::get_footer();
	}

	function terms_and_conditions_edit($terms_and_conditions_id = "")
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 13;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));
		...
		if (!empty($terms_and_conditions_id)) {
			$this->data['terms_and_conditions_master_data'] = $this->Terms_and_Conditions_Model->get_terms_and_conditions_master(array("terms_and_conditions_id" => $terms_and_conditions_id));
			if (empty($this->data['terms_and_conditions_master_data'])) {
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Terms_and_Conditions_Module/terms_and_conditions_edit', $this->data);
		parent::get_footer();
	}

	function userTermsandConditionsDoEdit()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 13;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));

		if (empty($_POST['terms_and_conditions_name'])) {
		...
		$terms_and_conditions_name = trim($_POST['terms_and_conditions_name']);
		$status = $_POST['status'];
		$is_exist = $this->Common_Model->getData(array('select' => '*', 'from' => 'terms_and_conditions_master', 'where' => "terms_and_conditions_name = '$terms_and_conditions_name' and terms_and_conditions_id != $terms_and_conditions_id"));
		//	echo $this->db->last_query();
		//	print_r($is_exist);
		if (!empty($is_exist)) {
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Terms and Conditions already exist in database.</div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			//echo $this->session->flashdata('alert_message' );
			//echo "anubhav";
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/terms-and-conditions-edit/" . $terms_and_conditions_id);
			exit;
		}

		$enter_data['terms_and_conditions_name'] = $terms_and_conditions_name;
		...
		if (!empty($terms_and_conditions_id)) {
			...
			$insertStatus = $this->Common_Model->update_operation(array('table' => 'terms_and_conditions_master', 'data' => $enter_data, 'condition' => "terms_and_conditions_id = $terms_and_conditions_id"));
			...
			$insertStatus = $this->Common_Model->add_operation(array('table' => 'terms_and_conditions_master', 'data' => $enter_data));
			if (!empty($insertStatus)) {
		...
		if (!empty($_POST['redirect_type'])) {
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/terms-and-conditions-edit");
		}

		REDIRECT(MAINSITE_Admin . $user_access->class_name . "/" . $user_access->function_name);
	}

	function userTermsandConditions_doUpdateStatus()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 13;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));
		...
		if ($user_access->update_module == 1) {
			...
			if (!empty($id_arr)) {
				...
				$response = $this->Common_Model->update_operation(array('table' => "terms_and_conditions_master", 'data' => $update_data, 'condition' => "terms_and_conditions_id in ($ids)"));
				if ($response) {
