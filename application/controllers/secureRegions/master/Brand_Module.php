...
class Brand_Module extends Main
{
	...
	function __construct()
	{
		...
		$this->load->model('administrator/master/Brand_Model');
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Brand_Module/list', $this->data);
		parent::get_footer();
	}
	...
	function brand_list()
	{
		...
		$data_count = $this->Brand_Model->get_brand_master($search);
		...
		$this->data['brand_master_data'] = $this->Brand_Model->get_brand_master($search);

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Brand_Module/brand_list', $this->data);
		parent::get_footer();
	}

	function brand_list_export()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 194;
		...
		$this->load->view('admin/master/Brand_Module/brand_list_export', $this->data);
	}

	function brand_view($brand_id = "")
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 194;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['brand_master_data'] = $this->Brand_Model->get_brand_master(array("brand_id" => $brand_id));
		if (empty($brand_id)) {
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Something Went Wrong. Please Try Again. anubhav</div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/" . $user_access->function_name);
		...
		$this->data['brand_master_data'] = $this->data['brand_master_data'][0];

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Brand_Module/brand_view', $this->data);
		parent::get_footer();
	}
	...
	function brand_edit($brand_id = "")
	{
		...
		if (!empty($brand_id)) {
			$this->data['brand_master_data'] = $this->Brand_Model->get_brand_master(array("brand_id" => $brand_id));
			if (empty($this->data['brand_master_data'])) {
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Brand_Module/brand_edit', $this->data);
		parent::get_footer();
	}

	function userBrandDoEdit()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 194;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));
		...
		$status = $_POST['status'];
		$is_exist = $this->Common_Model->getData(array('select' => '*', 'from' => 'brand_master', 'where' => "brand_name = '$brand_name' and brand_id != $brand_id"));
		//	echo $this->db->last_query();
		//	print_r($is_exist);
		if (!empty($is_exist)) {
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Brand already exist in database.</div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			//echo $this->session->flashdata('alert_message' );
			//echo "anubhav";
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/brand-edit/" . $brand_id);
			exit;
		}

		$enter_data['brand_name'] = $brand_name;
		...
		if (!empty($brand_id)) {
			...
			$insertStatus = $this->Common_Model->update_operation(array('table' => 'brand_master', 'data' => $enter_data, 'condition' => "brand_id = $brand_id"));
			...
			$insertStatus = $this->Common_Model->add_operation(array('table' => 'brand_master', 'data' => $enter_data));
			if (!empty($insertStatus)) {
		...
		if (!empty($_POST['redirect_type'])) {
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/brand-edit");
		}

		REDIRECT(MAINSITE_Admin . $user_access->class_name . "/" . $user_access->function_name);
	}

	function userBrand_doUpdateStatus()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 194;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));
		...
		if ($user_access->update_module == 1) {
			...
			if (!empty($id_arr)) {
				...
				$response = $this->Common_Model->update_operation(array('table' => "brand_master", 'data' => $update_data, 'condition' => "brand_id in ($ids)"));
				if ($response) {
