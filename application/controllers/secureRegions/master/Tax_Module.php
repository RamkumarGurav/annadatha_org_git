...
class Tax_Module extends Main
{
	...
	function __construct()
	{
		...
		$this->load->model('administrator/master/Tax_Model');
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Tax_Module/list', $this->data);
		parent::get_footer();
	}
	...
	function tax_list()
	{
		...
		$data_count = $this->Tax_Model->get_tax($search);
		...
		$this->data['tax_data'] = $this->Tax_Model->get_tax($search);

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Tax_Module/tax_list', $this->data);
		parent::get_footer();
	}

	function tax_list_export()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 5;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));
		...
		$this->load->view('admin/master/Tax_Module/tax_list_export', $this->data);
	}

	function tax_view($tax_id = "")
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 5;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['tax_data'] = $this->Tax_Model->get_tax(array("tax_id" => $tax_id));
		if (empty($tax_id)) {
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Something Went Wrong. Please Try Again. </div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/" . $user_access->function_name);
		...
		$this->data['tax_data'] = $this->data['tax_data'][0];

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Tax_Module/tax_view', $this->data);
		parent::get_footer();
	}
	...
	function tax_edit($tax_id = "")
	{
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Tax_Module/tax_edit', $this->data);
		parent::get_footer();
	}

	function userTaxDoEdit()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 5;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));

		if (empty($_POST['tax_name']) || empty($_POST['tax_percentage'])) {
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Something Went Wrong. Please Try Again. </div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/" . $user_access->function_name);
			exit;
		...
		$status = $_POST['status'];
		$is_exist = $this->Common_Model->getData(array('select' => '*', 'from' => 'tax', 'where' => "tax_name = '$tax_name' and tax_id != $tax_id"));
		//	echo $this->db->last_query();
		//	print_r($is_exist);
		if (!empty($is_exist)) {
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Tax already exist in database.</div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			//echo $this->session->flashdata('alert_message' );
			//echo "anubhav";
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/tax-edit/" . $tax_id);
			exit;
		}

		$enter_data['tax_name'] = $tax_name;
		$enter_data['tax_percentage'] = $tax_percentage;
		...
		if (!empty($tax_id)) {
			...
			$insertStatus = $this->Common_Model->update_operation(array('table' => 'tax', 'data' => $enter_data, 'condition' => "tax_id = $tax_id"));
			...
			$insertStatus = $this->Common_Model->add_operation(array('table' => 'tax', 'data' => $enter_data));
			if (!empty($insertStatus)) {
		...
		if (!empty($_POST['redirect_type'])) {
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/tax-edit");
		}

		REDIRECT(MAINSITE_Admin . $user_access->class_name . "/" . $user_access->function_name);
	}

	function userTax_doUpdateStatus()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 5;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));
		...
		if ($user_access->update_module == 1) {
			...
			if (!empty($id_arr)) {
				...
				$response = $this->Common_Model->update_operation(array('table' => "tax", 'data' => $update_data, 'condition' => "tax_id in ($ids)"));
				if ($response) {
